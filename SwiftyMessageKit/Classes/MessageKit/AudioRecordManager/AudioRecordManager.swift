...
class AudioRecordManager: NSObject {
    ...
    func updateMeters() {
        ...
        repeat {
            ...
            self.audioTimeInterval = NSNumber(value: NSNumber(value: recorder.currentTime).floatValue)
    ...
    func stopRecord() {
        ...
        // TODO: 录制音频小于3秒提示时间太短
        if (self.endTimer - self.startTime) < 3 {
            self.cancelRrcord()
            DispatchQueue.main.async(execute: {
                self.delegate?.audioRecordTooShort()
            })
        } else {
            self.audioTimeInterval = NSNumber(value: NSNumber(value: self.recorder?.currentTime ?? 0).int32Value)
