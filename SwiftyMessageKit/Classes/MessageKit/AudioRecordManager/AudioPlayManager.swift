...
/// The `AudioPlayerState` indicates the current audio controller state
...
open class AudioPlayManager: NSObject, AVAudioPlayerDelegate {
    ...
    open private(set) var state: AudioPlayerState = .stopped
    ...
    open func configureAudioCell(_ cell: AudioMessageCell, message: MessageType) {
        if playingMessage?.messageId == message.messageId, let collectionView = messageCollectionView, let player = audioPlayer {
            ...
            guard let displayDelegate = collectionView.messagesDisplayDelegate else {
                BFLog.error("MessagesDisplayDelegate has not been set.")
                return
            }
    ...
    open func playSound(for message: MessageType, in audioCell: AudioMessageCell) {
        ...
        case .audio(let item):
            ...
            BFLog.debug("create audio player for URL: \(item.url)")
            guard let player = try? AVAudioPlayer.init(contentsOf: item.url) else {
            ...
            do {
                ...
                BFLog.error("error localizedDescription: \(error.localizedDescription)")
                return
            }
            audioPlayer = player
        ...
        default:
            BFLog.info("BasicAudioPlayer failed play sound becasue given message kind is not Audio")
        }
    }
    ...
    private func startProgressTimer() {
        progressTimer?.invalidate()
        progressTimer = nil
        progressTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(AudioPlayManager.didFireProgressTimer(_:)), userInfo: nil, repeats: true)
    }
